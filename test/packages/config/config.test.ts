...
test("loadConfig applies default ollama embedding settings", () => {
  ...
  assert.equal(config.OLLAMA_TOPIC_NAMING_BASE_URL, "http://127.0.0.1:11434");
  assert.equal(config.OLLAMA_TOPIC_NAMING_MODEL, "qwen3.5:4b");
  assert.equal(config.OLLAMA_TOPIC_NAMING_TIMEOUT_MS, 30000);
});
...
test("loadConfig supports overriding ollama embedding settings", () => {
  const config = loadConfig({
    ...
    OLLAMA_TOPIC_NAMING_BASE_URL: "http://127.0.0.1:11436",
    OLLAMA_TOPIC_NAMING_MODEL: "qwen3.5:4b",
    OLLAMA_TOPIC_NAMING_TIMEOUT_MS: "45000",
  });
  ...
  assert.equal(config.OLLAMA_TOPIC_NAMING_BASE_URL, "http://127.0.0.1:11436");
  assert.equal(config.OLLAMA_TOPIC_NAMING_MODEL, "qwen3.5:4b");
  assert.equal(config.OLLAMA_TOPIC_NAMING_TIMEOUT_MS, 45000);
});
